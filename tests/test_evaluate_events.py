...
def test_evaluate_manifest_micro_benchmark() -> None:
    ...
    bundles = [
        module._load_video_bundle(manifest_path.parent, video_spec)
    ...
    assert results["summary"]["coverage"]["scene_types"] == [
        "indoor",
        "loading_dock",
        "office",
    ]
    ...
    assert (
        results["summary"]["events_by_type"]["loitering"]["mean_alert_latency_seconds"]
        == 10.0
    )
