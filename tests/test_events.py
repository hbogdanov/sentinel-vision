...
def test_intrusion_fires_only_on_entry() -> None:
    ...
    outside_track = Track(
        track_id=1, bbox=(20, 20, 30, 30), label="person", score=0.9, last_seen_frame=0
    )
    inside_track = Track(
        track_id=1, bbox=(1, 1, 9, 9), label="person", score=0.9, last_seen_frame=1
    )
    ...
    second = detector.evaluate(
        [inside_track], [zone], 1, timestamp + timedelta(seconds=1), 30.0, "cam_1"
    )
    third = detector.evaluate(
        [inside_track], [zone], 2, timestamp + timedelta(seconds=2), 30.0, "cam_1"
    )
...
def test_loitering_requires_dwell_time() -> None:
    ...
    zone = PolygonZone("restricted", [(0, 0), (10, 0), (10, 10), (0, 10)])
    track = Track(
        track_id=5, bbox=(1, 1, 9, 9), label="person", score=0.95, last_seen_frame=0
    )
    start = datetime(2026, 3, 8, tzinfo=timezone.utc)

    early = detector.evaluate([track], [zone], 0, start, 30.0, "cam_1")
    late = detector.evaluate(
        [track], [zone], 120, start + timedelta(seconds=4), 30.0, "cam_1"
    )
...
def test_intrusion_cooldown_allows_retrigger_after_exit_and_expiry() -> None:
    ...
    still_inside = detector.evaluate(
        [inside_track], [zone], 1, start + timedelta(seconds=1), 30.0, "cam_1"
    )
    exited = detector.evaluate(
        [outside_track], [zone], 2, start + timedelta(seconds=2), 30.0, "cam_1"
    )
    retrigger = detector.evaluate(
        [inside_track], [zone], 3, start + timedelta(seconds=3), 30.0, "cam_1"
    )
...
def test_loitering_uses_world_space_when_available() -> None:
    ...
    early = detector.evaluate([track], [zone], 0, start, 30.0, "cam_1")
    late = detector.evaluate(
        [track], [zone], 30, start + timedelta(seconds=2), 30.0, "cam_1"
    )
