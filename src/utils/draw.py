...
def draw_frame(
    ...
    for zone in zones:
        if isinstance(zone, PolygonZone):
            ...
            cv2.putText(frame, zone.name, tuple(points[0]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
            ...
            cv2.putText(frame, zone.name, start, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 215, 0), 2)
    ...
    for track in tracks:
        ...
        if track.track_id in dwell_map:
            ...
            cv2.putText(frame, dwell_text, (x1, min(frame.shape[0] - 10, y2 + 18)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
def _draw_trail(frame, history: list[tuple[int, int]], color: tuple[int, int, int]) -> None:
...
def _draw_alert_stack(frame, events: list[dict]) -> None:
    ...
    for idx, event in enumerate(reversed(visible_events)):
        ...
        cv2.putText(frame, text, (32, y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
