...
class GroundPlaneMapper:
    ...
    def from_correspondences(
        ...
        if matrix is None:
            raise ValueError("Failed to compute ground-plane homography from the provided correspondences.")
    ...
    def project_points(self, points: list[tuple[float, float]]) -> list[tuple[float, float]]:
        if not self.enabled:
            return []
