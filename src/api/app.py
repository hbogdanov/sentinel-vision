...
def health() -> HealthResponse:
    return HealthResponse(
        status="ok", db_path=str(store.db_path).replace("\\", "/"), alerts=store.count()
    )
...
def list_alerts(
    ...
    start_time: str | None = Query(
        default=None, description="Inclusive ISO-8601 UTC timestamp"
    ),
    ...
    return AlertListResponse(
        alerts=[AlertEvent.model_validate(alert) for alert in alerts],
        limit=limit,
        offset=offset,
    )
...
def get_alert(event_id: str) -> AlertEvent:
    ...
    if alert is None:
        raise HTTPException(
            status_code=404, detail=f"Alert '{event_id}' was not found."
        )
...
def list_cameras() -> CameraListResponse:
    return CameraListResponse(
        cameras=[
            CameraSummary.model_validate(camera) for camera in store.camera_summaries()
        ]
    )
...
def ingest_alert(event: AlertEvent) -> IngestResponse:
    ...
    return IngestResponse(
        stored=True, event_id=event.event_id, total_alerts=store.count()
    )
